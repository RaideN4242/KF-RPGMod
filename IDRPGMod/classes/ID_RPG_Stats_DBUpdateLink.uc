Class ID_RPG_Stats_DBUpdateLink extends UdpLink;

var string PendingLine;
var IpAddr A;
var ID_RPG_Mutator Mut;
var array<ID_RPG_Stats_Object> ToSave;
...
function PostBeginPlay()
{
	...
	if( BindPort(Rand(8000)+8000,true)>0 )
		Resolve(Class'ID_RPG_Mutator'.Default.RemoteDatabaseURL);
}
event Resolved( IpAddr Addr )
{
	A = Addr;
	A.Port = Class'ID_RPG_Mutator'.Default.RemotePort;
...
event ReceivedText( IpAddr Addr, string Text )
{
	...
	case ENetID.ID_RequestPassword:
		...
		SendText(A,Chr(ENetID.ID_HeresPassword)$Class'ID_RPG_Mutator'.Default.RemotePassword);
		break;
