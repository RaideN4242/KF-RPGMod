// Written by Marco
class ID_Weapon_Base_Turret_Sentry extends ID_Weapon_Base_Turret_Sentry_Base // Gay hack, but has to be done.
	Placeable
	Config(USCMSentry)
	CacheExempt;

#exec obj load file="WPCAlienWeap_T.utx"
#exec obj load file="WPCAlienWeap_S.uax"
#exec obj load file="KF_LAWSnd.uax"


var transient ID_Weapon_Base_Turret_MuzzleFlash Flashes[4];
...
//var Pawn OwnerPawn;
//var ID_Weapon_Base_Turret WeaponOwner;
...
/*
final function SetOwningPlayer( Pawn Other, ID_Weapon_Base_Turret Wep )
{
	OwnerPawn = Other;
	...
	HealthMax += class'ID_Skill_TurretHP'.static.GetAddHP(ID_RPG_Base_HumanPawn(Other));
	Health = HealthMax;
}*/
	...
	if( Level.NetMode!=NM_Client && Physics==PHYS_Karma )
	{
		...
		if( KParams!=None && KParams.bContactingLevel && NextFlipCheckTime<Level.TimeSeconds )
		{
			...
			if( bFlip!=bIsCurrentlyFlipped )
			{
				...
				ID_Weapon_Base_Turret_Sentry_AI(Controller).NotifyGotFlipped(bFlip);
			}
		}
	}
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, class<DamageType> damageType, optional int HitIndex )
{
	...
	if( Health<=0 )
	{
		...
		ID_Weapon_Base_Turret_Sentry_AI(Controller).NotifyGotFlipped(true);
	}
}
...
simulated function PlayTurretDied()
{
	...
	Skins[0] = Texture'WPCAlienWeap_T.sentrygun';
	SetTimer(0,false);
}
...
simulated function Timer()
{
	...
	if( (Level.TimeSeconds-LastRenderTime)<4 )
	{
		...
		{
			Flashes[FXBarrelNumber] = Spawn(Class'ID_Weapon_Base_Turret_MuzzleFlash');
			AttachToBone(Flashes[FXBarrelNumber],BarrelBones[FXBarrelNumber]);
			Flashes[FXBarrelNumber+1] = Spawn(Class'ID_Weapon_Base_Turret_MuzzleFlash');
...
defaultproperties
{
     ...
     MenuName="Turret"
     ControllerClass=Class'IDRPGMod.ID_Weapon_Base_Turret_Sentry_AI'
     bStasis=False
     Mesh=SkeletalMesh'WPCAlienWeap_A.USCMSentryGun'
     Skins(0)=Texture'WPCAlienWeap_T.Weapons.sentrygun'
     ...
     End Object
     KParams=KarmaParamsRBFull'IDRPGMod.ID_Weapon_Base_Turret_Sentry.KarmaParamsRBFull0'
