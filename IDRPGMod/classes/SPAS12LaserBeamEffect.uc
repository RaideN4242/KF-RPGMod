...
//=============================================================================
class SPAS12LaserBeamEffect extends xEmitter;
	...
	else
	{
		...
		else
		{
			...
			if (Attachment != None && (Level.TimeSeconds - Attachment.LastRenderTime) < 1)
			{
				...
				NewRotation = Rotator(-Attachment.GetBoneCoords('LightBone').XAxis);
				SetLocation( StartEffect + Attachment.GetBoneCoords('LightBone').XAxis * LaserDist );
			}
			else
			{
