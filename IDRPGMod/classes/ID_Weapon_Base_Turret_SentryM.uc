// Written by Marco
class ID_Weapon_Base_Turret_SentryM extends ID_Weapon_Base_Turret_Sentry_Base // Gay hack, but has to be done.
	Placeable
	Config(IDRPGTurretM)
	CacheExempt;
...
//var() globalconfig int EffectiveRange;
//var() globalconfig float FireRateTime;
var() globalconfig float PeripheralVision;
var() globalconfig bool bCanBeBaseForPawns;
var() class<DamageType> HitDgeType;
var vector LaserOffset;
var KFNewTracer mTracer[4];
var transient Font HUDFontz[2];
//var Pawn OwnerPawn;
//var ID_Weapon_Base_TurretM WeaponOwner;
...
/*
final function SetOwningPlayer( Pawn Other, ID_Weapon_Base_TurretM Wep )
{
	OwnerPawn = Other;
	...
	if( Level.NetMode!=NM_Client && Physics==PHYS_Karma )
	{
		...
		if( KParams!=None && KParams.bContactingLevel && NextFlipCheckTime<Level.TimeSeconds )
		{
			...
			if( bFlip!=bIsCurrentlyFlipped )
			{
				...
				ID_Weapon_Base_Turret_Sentry_AIM(Controller).NotifyGotFlipped(bFlip);
			}
		}
	}
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, class<DamageType> damageType, optional int HitIndex )
{
	...
	if( Health<=0 )
	{
		...
		ID_Weapon_Base_Turret_Sentry_AIM(Controller).NotifyGotFlipped(true);
	}
}
	...
	for( i=0; i<2; ++i )
	{
		...
		foreach TraceActors(Class'Actor', Res, HL, HN, Start+X * EffectiveRange, Start)
		{
			if( Res!=Self && (Res==Level || Res.bBlockActors || Res.bProjTarget || Res.bWorldGeometry) && (KFPawn(Res)!=None/* ||!bevilturret*/)
				&& KFBulletWhipAttachment(Res)==None  && ID_Weapon_Base_Turret_SentryM(Res)==None)
			{
				HitA = Res;
				break;
			}
		...
		if( HitA!=None && KFHumanPawn(HitA)!=None )
		{			
...
{
	// Make noise
	PlaySound(FireSnd,SLOT_Pain,2.f,,800.f);
	...
	if( (Level.TimeSeconds-LastRenderTime)<4 )
	{
		...
		//{
			Flashes[FXBarrelNumber] = Spawn(Class'KFMod.HealingFX');
			AttachToBone(Flashes[FXBarrelNumber],BarrelBones[FXBarrelNumber]);
			Flashes[FXBarrelNumber+1] = Spawn(Class'KFMod.HealingFX');
		...
		else
		{
			...
			Flashes[FXBarrelNumber].Trigger(none, none);
			Flashes[FXBarrelNumber+1].Trigger(none, none);
		}*/
	}
	FXBarrelNumber+=2;
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
	...
	if( WeaponOwner!=None )
	{
		if( OwnerPawn!=None && PlayerController(OwnerPawn.Controller)!=None )
			PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'ID_Message_TurretM',2);
...
defaultproperties
{
     ...
     BarrelBones(2)="tip"
     BarrelBones(3)="tip"
     FireSnd=SoundGroup'KF_MP7Snd.Medicgun_Fire'
     PeripheralVision=0.700000
     LaserOffset=(X=12.000000,Z=11.800000)
     HitDamages=30
     TurretHealth=600
     EffectiveRange=5000
     FireRateTime=0.500000
     Team=250
     VehicleNameString="Medic RPG Turret"
     MenuName="Turret"
     ControllerClass=Class'IDRPGMod.ID_Weapon_Base_Turret_Sentry_AIM'
     bStasis=False
     Mesh=SkeletalMesh'DZResPack.USCMSentryGun'
     Skins(0)=Texture'DZResPack.sentrygunM'
     ...
     Begin Object Class=KarmaParamsRBFull Name=KarmaParamsRBFull0
         ...
         bDoSafetime=True
         KFriction=0.850000
         KImpactThreshold=350.000000
     End Object
     KParams=KarmaParamsRBFull'IDRPGMod.ID_Weapon_Base_Turret_SentryM.KarmaParamsRBFull0'
