class ID_Weapon_Base_TurretM extends ID_Weapon_Base_Turret_Base
	Config(IDRPGTurretM)
	abstract;
	
#exec obj load file="DZResPack.u"
	
//var ID_Weapon_Base_Turret_SentryM CurrentSentry;
...
var class<ID_Weapon_Base_Turret_SentryM> SentryClass;
...
simulated function Destroyed()
{
	...
	{
		// Hack to switch to another weapon on the client when we throw the ID_Weapon_Base_Turret_Sentry bot is destroyed.
...
event ServerStartFire(byte Mode)
{
	...
	if( CurrentSentry==None )
	{
		...
		if( FastTrace(Spot,Instigator.Location) )
		{
			...
			if( CurrentSentry!=None )
			{
				if( PlayerController(Instigator.Controller)!=None )
					PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'ID_Message_TurretM',1);
		...
		if( PlayerController(Instigator.Controller)!=None )
			PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'ID_Message_TurretM',0);
	}
}
*/
...
defaultproperties
{
     SentryClass=Class'IDRPGMod.ID_Weapon_Base_Turret_SentryM'
     SentryStationaryClass=Class'IDRPGMod.ID_Weapon_Base_Turret_SentryM'
     MagCapacity=1
     HudImage=Texture'DZResPack.SentryGun_Unselected'
     SelectedHudImage=Texture'DZResPack.SentryGun_Selected'
     Weight=0.000000
     bModeZeroCanDryFire=True
     SleeveNum=0
     TraderInfoTexture=Texture'DZResPack.SentryGun_Trader'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'IDRPGMod.ID_Weapon_Base_Turret_PickupM'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'IDRPGMod.ID_Weapon_Base_Turret_AttachmentM'
     IconCoords=(X1=245,Y1=39,X2=329,Y2=79)
     ItemName="Medic RPG Turret"
     Mesh=SkeletalMesh'DZResPack.SentryLaptop'
     Skins(1)=Texture'DZResPack.SentryLaptopNEW'
     AmbientGlow=35
     MessageClass=Class'IDRPGMod.ID_Message_TurretM'
}
