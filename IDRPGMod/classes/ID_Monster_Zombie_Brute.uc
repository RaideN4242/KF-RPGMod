class ID_Monster_Zombie_Brute extends ID_Monster_Zombie_Brute_Base;
...
Ignores StartCharging;
	...
	function EndState()
	{
		...
		ID_Monster_Zombie_Brute_Controller(Controller).RageFrustrationTimer = 0;
	...
	function Bump(Actor Other)
	{
		...
		if (!bShotAnim && KFM != None && ID_Monster_Zombie_Brute(Other) == None && Pawn(Other).Health > 0)
...
{
	return (ID_Monster_Zombie_Brute(P) != none);
}
...
function PlayHit(float Damage, Pawn InstigatedBy, vector HitLocation, class<DamageType> damageType, vector Momentum, optional int HitIdx)
{
	local Actor A;
	if (bBlockedHS)
		A = Spawn(class'ID_Monster_Zombie_Brute_BlockHitEmitter', InstigatedBy,, HitLocation, rotator(Normal(HitLocation - Location)));
	else
...
defaultproperties
{
     DetachedArmClass=Class'IDRPGMod.ID_Monster_Zombie_Brute_SeveredArm'
     DetachedLegClass=Class'IDRPGMod.ID_Monster_Zombie_Brute_SeveredLeg'
     DetachedHeadClass=Class'IDRPGMod.ID_Monster_Zombie_Brute_SeveredHead'
     ControllerClass=Class'IDRPGMod.ID_Monster_Zombie_Brute_Controller'
}
