class ID_Monster_Zombie_Shiver extends ID_Monster_Zombie_Shiver_Base;
...
function FlashTeleport()
{
	...
	for (iEndAngle = 0; iEndAngle < MaxTeleportAngles; iEndAngle++)
	{
		...
		for (iAttempts = 0; iAttempts < MaxTeleportAttempts; iAttempts++)
		{
			...
			NewLoc = Target.Location - vector(RotNew) * LandTargetDist;
			...
			if (SetLocation(NewLoc))
			{
				...
				if (Controller.PointReachable(NewLoc))
				{
					Velocity = vect(0, 0, 0);
