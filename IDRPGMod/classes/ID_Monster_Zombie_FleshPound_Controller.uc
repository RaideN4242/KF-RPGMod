class ID_Monster_Zombie_FleshPound_Controller extends ID_RPG_Base_Monster_Controller;
...
ignores EnemyNotVisible;
	...
	function DoSpinDamage()
	{
		...
		foreach CollidingActors(class'actor', A, (ID_Monster_Zombie_FleshPound(pawn).MeleeRange * 1.5)+pawn.CollisionRadius, pawn.Location)
			ID_Monster_Zombie_FleshPound(pawn).SpinDamage(A);
	}
...
state ZombieCharge
{
	...
	{
		local ID_Monster_Zombie_FleshPound ZFP;
		Global.Tick(Delta);
		...
		if( RageFrustrationTimer < RageFrustrationThreshhold )
		{
			...
			if( RageFrustrationTimer >= RageFrustrationThreshhold )
			{
				ZFP = ID_Monster_Zombie_FleshPound(Pawn);
