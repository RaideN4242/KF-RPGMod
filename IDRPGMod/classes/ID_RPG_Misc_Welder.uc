class ID_RPG_Misc_Welder extends ID_RPG_Base_Weapon_Melee
...
simulated function Tick(float dt)
{
	...
	if (ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor != none && VSize(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor.Location - Owner.Location) <= (weaponRange * 1.5) )
	{
		bNoTarget = false;
		if ( KFDoorMover(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor) != none)
			ScreenWeldPercent = KFDoorMover(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor).WeldStrength /
												KFDoorMover(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor).MaxWeld * 100;
		else if(DemoSentry(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor)!=None)
			ScreenWeldPercent = DemoSentry(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor).Health/
												DemoSentry(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor).SentryHealth * 100;
		else
			ScreenWeldPercent = SVehicle(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor).Health/
												SVehicle(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor).HealthMax * 100;
		if( ScriptedScreen==None )
			InitMaterials();
	...
	else if (ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor == none || ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor != none && VSize(ID_RPG_Misc_Welder_Fire(FireMode[FireModeArray]).LastHitActor.Location - Owner.Location) > (weaponRange * 1.5) && !bNoTarget  )
	{
		if( ScriptedScreen==None )
			InitMaterials();
		ScriptedScreen.Revision++;
...
defaultproperties
{
     ...
     SleeveNum=2
     FireModeClass(0)=Class'IDRPGMod.ID_RPG_Misc_Welder_Fire'
     FireModeClass(1)=Class'IDRPGMod.ID_RPG_Misc_Welder_AltFire'
     AIRating=-2.000000
     bMeleeWeapon=False
     ...
     InventoryGroup=5
     GroupOffset=1
     PickupClass=Class'IDRPGMod.ID_RPG_Misc_Welder_Pickup'
     BobDamping=6.000000
