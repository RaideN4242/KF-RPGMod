// A very simple AI!
Class ID_Weapon_Base_Turret_Sentry_AIM extends AIController;

var ID_Weapon_Base_Turret_SentryM Turret;
var transient float NextShotTime;

function Restart()
{
	Super.Restart();
	Turret = ID_Weapon_Base_Turret_SentryM(Pawn);
	GoToState('Idle');
}

function bool SetEnemy( Pawn Other )
{
	if( Other==None || Other.bDeleteMe || Other.Health>=Other.HealthMax || Turret.SameSpeciesAs(Other) || !CanSpotEnemy(Other) )
		Return False;
	Enemy = Other;
	Return True;
}
...
Ignores SeePlayer,SeeMonster,HearNoise;
	...
	function Timer()
	{
		...
		if( Enemy==None || Enemy.Health>=Enemy.HealthMax || !CanSpotEnemy(Enemy) )
		{
			GoToState('Searching');
			Return;
		}
