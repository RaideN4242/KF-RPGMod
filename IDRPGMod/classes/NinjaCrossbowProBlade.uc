class NinjaCrossbowProBlade extends ROBallisticProjectile;
...
simulated state OnWall
{
	...
	function ProcessTouch (Actor Other, vector HitLocation)
	{
		...
		if( Pawn(Other)!=None && Pawn(Other).Inventory!=None )
		{
			...
			{
				if( NinjaCrossbowPro(Inv)!=None && Weapon(inv).AmmoAmount(0)<Weapon(inv).MaxAmmo(0) )
				{
...
defaultproperties
{
     DamageTypeHeadShot=Class'IDRPGMod.DamTypeNinjaCrossbowProHeadShot'
     HeadShotDamageMult=2.000000
     AmbientSoundRef="KF_IJC_HalloweenSnd.KF_SawbladeBow_Projectile_Loop"
     StaticMeshRef="DZResPack.Star"
     ...
     MyDamageType=Class'IDRPGMod.DamTypeNinjaCrossbowPro'
     ...
     LifeSpan=4.000000
     DrawScale=3.000000
     Skins(0)=Combiner'DZResPack.Box_CMB'
     Style=STY_Alpha
     bUnlit=False
     SoundVolume=175
