class ID_Weapon_Base_Turret extends ID_Weapon_Base_Turret_Base
	Config(USCMSentry)
	abstract;

//var ID_Weapon_Base_Turret_Sentry CurrentSentry;
...
var class<ID_Weapon_Base_Turret_Sentry> SentryClass;
...
event ServerStartFire(byte Mode)
{
	...
	if( CurrentSentry==None )
	{
		...
		if( FastTrace(Spot,Instigator.Location) )
		{
			...
			if( CurrentSentry!=None )
			{
				if( PlayerController(Instigator.Controller)!=None )
					PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'ID_Message_Turret',1);
		...
		if( PlayerController(Instigator.Controller)!=None )
			PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'ID_Message_Turret',0);
	}
}
*/
...
defaultproperties
{
     SentryClass=Class'IDRPGMod.ID_Weapon_Base_Turret_Sentry'
     SentryStationaryClass=Class'IDRPGMod.ID_Weapon_Base_Turret_Sentry'
     MagCapacity=1
     HudImage=Texture'WPCAlienWeap_T.HUD.SentryGun_Unselected'
     SelectedHudImage=Texture'WPCAlienWeap_T.HUD.SentryGun_Selected'
     Weight=3.000000
     bModeZeroCanDryFire=True
     SleeveNum=0
     TraderInfoTexture=Texture'WPCAlienWeap_T.Trader.Trader_SentryGun'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'IDRPGMod.ID_Weapon_Base_Turret_Pickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'IDRPGMod.ID_Weapon_Base_Turret_Attachment'
     IconCoords=(X1=245,Y1=39,X2=329,Y2=79)
     ItemName="Turret"
     Mesh=SkeletalMesh'WPCAlienWeap_A.SentryLaptop'
     Skins(1)=Combiner'WPCAlienWeap_T.Weapons.SentryLaptopFinal'
     AmbientGlow=35
}
