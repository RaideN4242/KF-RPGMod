...
state LaunchMine
{
	simulated function BeginState()
	{
		...
		if( Physics!=PHYS_Projectile )
		{
			...
			if( Level.NetMode!=NM_DedicatedServer )
			{
				TweenAnim('Up',0.1f);
				PlaySound(Sound'blade_in');
				PlaySound(Sound'combine_mine_deploy1',SLOT_Talk);
			}
			SetTimer(0.6,false);
		}
		bNetNotify = false;
	...
	simulated function Timer()
	{
		if( !bPreLaunched )
		{
			bPreLaunched = true;
			if( Level.NetMode!=NM_DedicatedServer )
				PlaySound(Sound'rmine_blip3',SLOT_Misc);
			SetTimer(0.2,false);
		}
...
defaultproperties
{
     ...
     MyDamageType=Class'IDRPGMod.DamTypeHopMine'
     ...
     LifeSpan=0.000000
     Mesh=SkeletalMesh'DZResPack.HopMineM'
     DrawScale=0.500000
     AmbientGlow=25
     bUnlit=False
