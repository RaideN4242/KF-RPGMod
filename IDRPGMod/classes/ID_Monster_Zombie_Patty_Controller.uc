class ID_Monster_Zombie_Patty_Controller extends ID_RPG_Base_Monster_Controller;
...
function FightEnemy(bool bCanCharge)
{
	...
	if ( !EnemyVisible() )
	{
		...
		if( bAlreadyFoundEnemy || ID_Monster_Zombie_Patty(Pawn).SneakCount > 2 )
		{
		...
		{
			// Added sneakcount hack to try and fix the endless loop crash. Try and track down what was causing this later - Ramm
			ID_Monster_Zombie_Patty(Pawn).SneakCount++;
...
WaitForAnim:
	...
	if ( ID_Monster_Zombie_Patty(Pawn).bShotAnim )
	{
		Goto('Moving');
	}
...
Begin:
	...
	if( HidingSpots==None )
		ID_Monster_Zombie_Patty(Pawn).BeginHealing();
